...
bool match(ifstream& in, char* str) {  // makes sure that str is equal to what is pulled from the document. 
    for (; *str != '\0'; ++str)
        if (*str != in.get())
            return false;
    return true;
...
unsigned int v2l(int i) { // maps a literal as it appears in the cnf to literal
	if (i < 0) return ((-i) << 1) - 1;   // that means we return (2*-i -1), which represents the negation of 2*i
	else return i << 1; // returns 2i if the literal isn't in the negated form. 
} 

Var l2v(Lit l) {	// basically does the opposite of the previous function, v2l. 
	return (l+1) / 2;	
} 

Lit negate(Lit l) {
	if (Neg(l)) return l + 1;  // if the literal is represented by an odd number, to represent it's negated value add 1.  
	return l - 1;		
}
...
class Clause {
	clause_t c;				// reminder, c is of type vector<Lit> , Lit is just a different name for int. 
	int lw,rw; //watches;	
public:	
	Clause(){};
...
class Solver {
	...
	//------------------------------------------ parameters we added for the LR MAB implementation -------------------------------------------------------------------
	vector<int> participated;
	vector<double> Q_vals;
	int alpha; 
	vector<int> assigned; // saves the point in time aka current amount of learned clauses when each variable got an assignment. 
	//------------------------------------------ parameters we added for the LR MAB implementation -------------------------------------------------------------------
