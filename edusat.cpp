...
void Solver::read_cnf(ifstream& in) {
	...
	cnf.reserve(clauses);   // reserves a vector of size "clauses", which is the number of clauses in SAT formula we want to solve. 
...
void Solver::initialize() {	
	//-------------------------------------------------------------- parameters we added for the heuristic implementation ----------------------------------------
	alpha = 0.4;

	participated.resize(nvars + 1); //  records the number of times a var participated in learning new clause. 
	Q_vals.resize(nvars + 1);
	fill(Q_vals.begin(), Q_vals.end(), 0); // initializing the Qval (of the heuristic) of all variables to 0.
...
inline void Solver::assert_lit(Lit l) {
	trail.push_back(l);
	int var = l2v(l);
	assigned[var] = num_learned; // var is assigned, so we update the point in time it happened for the heuristic calculations. 
	participated[var] = 0; // var's participation is counting starts again. 
...
void Solver::m_rescaleScores(double& new_score) {
	...
	cout << "currently entering loop: " << endl;
...
SolverState Solver::decide(){
	...
	case VAR_DEC_HEURISTIC::BANDIT_BASIC: {
		double Qmax = 0;
		Var varMax = 1;
		...
		/*cout << "Variable: " << varMax;
		cout << endl;
		cout << "Q max: " << Qmax;
		cout << endl;*/
		best_lit = getVal(varMax);
...
int Solver::analyze(const Clause conflicting) {
	...
	do {
		for (clause_it it = current_clause.cl().begin(); it != current_clause.cl().end(); ++it) {
			Lit lit = *it;
			v = l2v(lit);
			cout << "The literal is: " << lit << endl;
			cout <<"This is the variable we're currently looking at: " << v << endl;
			if (!marked[v]) {
				marked[v] = true;
		...
		marked[v] = false;
		/* add this variable to the vector that stores all variables that participated in learning the conflict clause. */
		participated[v] += 1;
		--resolve_num; 
		participated[v] += 1;
		if(!resolve_num) continue; // essentially break from the loop, because resolve_num <= 0. 
	...
	++num_learned; // LearntCounter from the paper. 
	...
	/* AfterConflictAnalysis should be implemented here. */
	if (alpha > 0.06) {
		alpha -= 1e-6;
	}
	return bktrk; 
...
void Solver::backtrack(int k) {
	...
	for (trail_t::iterator it = trail.begin() + separators[k+1]; it != trail.end(); ++it) { // erasing from k+1
		Var v = l2v(*it);
		cout << "v is "<< v<< " d level: " << dlevel[v]<< endl;
		if (dlevel[v]) { // we need the condition because of learnt unary clauses. In that case we enforce an assignment with dlevel = 0.
			...
			interval = num_learned - assigned[v];
			cout << "interval: " << interval << endl;
			if (interval > 0) {
				r = participated[v] / interval;
				Q_vals[v] = (1 - alpha) * Q_vals[v] + alpha * r;
			}
